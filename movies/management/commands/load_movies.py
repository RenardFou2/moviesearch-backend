...
class Command(BaseCommand):
    help = 'Load movie data from tmdb_5000_movies.csv or dynamically fetch from TMDB API'
    ...
    def parse_date(self, date_string):
        """
        Parse a date string and return a Python date object.
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING(
                f"Invalid date format: {date_string}. Skipping."
            ))
            return None
    ...
    def fetch_from_tmdb(self):
        """Fetch movies from TMDB by category and populate the database."""
        categories = {
            "Action": 28,
            "Comedy": 35,
            "Drama": 18,
        ...
        for category_name, genre_id in categories.items():
            ...
            page = 1
            while page <= 5:  # Przeszukuje x stron wyników z TMDB
                ...
                if response.status_code != 200:
                    self.stdout.write(self.style.ERROR(
                        f"Failed to fetch movies for {category_name}. Status code: {response.status_code}"
                    ))
                    break
                ...
                for movie_data in movies:
                    try:
                        ...
                        self.stdout.write(self.style.ERROR(
                            f"Error processing movie {movie_data['title']} ({movie_data['id']}): {e}"))
